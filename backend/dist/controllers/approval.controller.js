...
async function getUserIdForEmail(client, email, role) {
    ...
    if (userResult.rows.length === 0) {
        ...
        const fallbackUserId = (0, crypto_1.randomUUID)();
        await client.query(`INSERT INTO "User" (id, email, name, role, "createdAt", "updatedAt")
			 VALUES ($1, $2, $3, $4, NOW(), NOW())
			 ON CONFLICT (email) DO UPDATE SET "updatedAt" = NOW()
			 RETURNING id`, [fallbackUserId, email, email.split("@")[0], role]);
...
async function getApprovalQueue(req, res, next) {
    ...
    try {
        ...
        const approvalActionsResult = await client.query(`SELECT
			 ...
			 ORDER BY aa."createdAt" ASC`, [queueResult.rows.map((row) => row.id)]);
...
async function approveRequest(req, res, next) {
    ...
    try {
        ...
        await client.query(`UPDATE "VenueRequest"
			 ...
			 WHERE id = $3`, [nextStatus, nextApproverId, requestRecord.id]);
        await client.query(`INSERT INTO "ApprovalAction" (id, "requestId", "approverId", action, remarks, "createdAt")
...
async function requestRevision(req, res, next) {
    ...
    try {
        ...
        await client.query(`UPDATE "VenueRequest"
			 ...
			 WHERE id = $2`, [requestRecord.requesterId, requestRecord.id]);
        await client.query(`INSERT INTO "AuditLog" (id, "requestId", "performedById", action, details, "ipAddress", "createdAt")
			 VALUES ($1, $2, $3, $4, $5::jsonb, $6, NOW())`, [
            (0, crypto_1.randomUUID)(),
            requestRecord.id,
...
async function getArchive(req, res, next) {
    ...
    try {
        ...
        const approvalActionsResult = await client.query(`SELECT
			 ...
			 INNER JOIN "User" u ON u.id = aa."approverId"
			 WHERE aa."requestId" = ANY($1::text[])
			 ORDER BY aa."createdAt" ASC`, [archiveResult.rows.map((row) => row.id)]);
        const approvalActionsByRequestId = new Map();
        for (const action of approvalActionsResult.rows) {
            const requestActions = approvalActionsByRequestId.get(action.requestId) ?? [];
            requestActions.push({
