...
function formatTimeForDss(value: Date): string {
	...
	try {
		...
		if (requestId) {
			const requestResult = await client.query(
				`SELECT id, "venueId", "ministryId", "startDateTime", "endDateTime", attendees, attachments, signatures
				 FROM "VenueRequest"
				 WHERE id = $1
				 LIMIT 1`,
				[requestId],
			...
			const requestRecord = requestResult.rows[0] as {
				id: string;
				venueId: string;
				ministryId: string;
				startDateTime: Date;
